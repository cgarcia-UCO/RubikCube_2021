...
class Cube:
    ...
    def __init__(self, N=3, plastic_color=None, face_colors=None, arrays=None):
        self.N = N
        ...
        self._move_list = []
        self._initialize_arrays(arrays=arrays)

    def _initialize_arrays(self, arrays=None):
        ...
        for i in range(6):
            ...
            if arrays == None:
                ...
                #TODO aquí se inicializan los colores del cubo
                #colors_i = np.random.randint(6, size=face_centroids_t.shape[0])
            else:
                colors_i = arrays[i].copy()
                colors_i = [item for sublist in colors_i for item in sublist]
                colors_i = np.asarray(colors_i)
